...
class RolloutStorage:
    ...
    def modify_storage_size(self, data_size):
        """
        Some storage space is not used due to early termination of episodes
        data_size: total num of transitions used to train networks for this training iteration
        """
        self.observations = self.observations[:data_size, :, :]
        self.states = self.states[:data_size, :, :]
        self.rewards = self.rewards[:data_size, :, :]
        self.actions = self.actions[:data_size, :, :]
        self.dones = self.dones[:data_size, :, :]

        # For PPO
        self.actions_log_prob = self.actions_log_prob[:data_size, :, :]
        self.values = self.values[:data_size, :, :]
        self.returns = self.returns[:data_size, :, :]
        self.advantages = self.advantages[:data_size, :, :]
        self.mu = self.mu[:data_size, :, :]
        self.sigma =  self.sigma[:data_size, :, :]     
        
        self.num_transitions_per_env = data_size
    ...
    def mini_batch_generator(self, num_mini_batches, batch_size=None, mini_batch_size=None):
        if mini_batch_size is None:
            batch_size = self.num_envs * self.num_transitions_per_env
            mini_batch_size = batch_size // num_mini_batches
        print("batch_size, mini_batch_size:", batch_size, mini_batch_size)
